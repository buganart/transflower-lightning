...
flags.DEFINE_bool("visualize", False, "Whether to visualize the cameras for debugging.")
random.seed(0)
np.random.seed(0)
...
def plot_cameras(cgroup):
    points_world = np.array(
        [
            [40.0, 0.0, 0.0],  # arrow x: red
            [0.0, 40.0, 0.0],  # arrow y: green
            [0.0, 0.0, 40.0],  # arrow z: blue
        ]
    )
    colors = ["r", "g", "b"]
    axes_all = [
        vedo.Arrows([[0, 0, 0]], [points_world[i]]).c(colors[i]) for i in range(3)
    ]
...
def main(_):
    ...
    for env_name, seq_names in aist_dataset.mapping_env2seq.items():
        ...
        if FLAGS.visualize:
            ...
            vedo.show(
                *axes_all, vedo.Points(keypoints3d, r=12), interactive=True, axes=True
            )
            vedo.clear()
