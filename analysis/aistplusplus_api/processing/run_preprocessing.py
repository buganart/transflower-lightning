...
def load_keypoints2d(data_dir, seq_name, njoints=17):
    ...
    video_names = [
        AISTDataset.get_video_name(seq_name, view) for view in AISTDataset.VIEWS
    ]
    ...
    keypoints2d = np.array(keypoints2d, dtype=np.float32)  # (nviews, N, njoints, 3)
    det_scores = np.array(det_scores, dtype=np.float32)  # (nviews, N)
...
def main(_):
    ...
    video_names = [
        video_name for video_name in video_names if len(video_name.split("_")) == 6
    ]
    seq_names = list(
        set([AISTDataset.get_seq_name(video_name)[0] for video_name in video_names])
    )
