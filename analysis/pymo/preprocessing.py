...
from analysis.pymo.rotation_tools import (
    Rotation,
    euler2expmap,
    euler2expmap2,
    expmap2euler,
        ...
        for track in X:
            ...
            for joint in track.traverse():
                ...
                pos_df["%s_Xposition" % joint] = pd.Series(
                    data=[e[0] for e in tree_data[joint][1]], index=pos_df.index
                )
                pos_df["%s_Yposition" % joint] = pd.Series(
                    data=[e[1] for e in tree_data[joint][1]], index=pos_df.index
                )
                pos_df["%s_Zposition" % joint] = pd.Series(
                    data=[e[2] for e in tree_data[joint][1]], index=pos_df.index
                )
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            # end of abdolute_translation_deltas
                ...
                forward = (
                    forward / np.sqrt((forward ** 2).sum(axis=-1))[..., np.newaxis]
                )
...
class ConstantsRemover(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if self.only_cols is not None:
            self.const_dims_ = [
                c for c in cols if ((stds[c] < self.eps).any()) and c in self.only_cols
            ]
        else:
        ...
        self.const_values_ = {
            c: X[0].values[c].values[0] for c in cols if self.const_dims_
        }
        return self
