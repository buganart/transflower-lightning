...
def render_mp4(
    mocap_track, filename, data=None, ax=None, axis_scale=50, elev=45, azim=45
):
    if ax is None:
    ...
    def animate(frame):
        ...
        for joint in mocap_track.skeleton.keys():
            ...
            for c in children_to_draw:
                ...
                lines[0][j].set_data(
                    np.array([[child_x, parent_x], [-child_z, -parent_z]])
                )
    ...
    ani = animation.FuncAnimation(
        fig, animate, np.arange(data.shape[0]), interval=1000 / fps
    )
