...
imp.reload(analysis.pymo)
import imp

imp.reload(analysis.pymo.preprocessing)
...
euler2expmap(
    (
        data.values["LeftFoot_Zrotation"][1],
        data.values["LeftFoot_Xrotation"][1],
        data.values["LeftFoot_Yrotation"][1],
    ),
    "ZXY",
    True,
)
e1 = euler2expmap(
    (
        data2.values["LeftFoot_Zrotation"][0],
        data2.values["LeftFoot_Xrotation"][0],
        data2.values["LeftFoot_Yrotation"][0],
    ),
    "ZXY",
    True,
)
e2 = euler2expmap(
    (
        data2.values["LeftFoot_Zrotation"][1],
        data2.values["LeftFoot_Xrotation"][1],
        data2.values["LeftFoot_Yrotation"][1],
    ),
    "ZXY",
    True,
)
e3 = euler2expmap(
    (
        data2.values["LeftFoot_Zrotation"][2],
        data2.values["LeftFoot_Xrotation"][2],
        data2.values["LeftFoot_Yrotation"][2],
    ),
    "ZXY",
    True,
)
...
(
    out_data[0].values["LeftFoot_alpha"] ** 2
    + out_data[0].values["LeftFoot_beta"] ** 2
    + out_data[0].values["LeftFoot_gamma"] ** 2
).mean()
(
    out_data2[0].values["LeftFoot_alpha"] ** 2
    + out_data2[0].values["LeftFoot_beta"] ** 2
    + out_data2[0].values["LeftFoot_gamma"] ** 2
).mean()
out_data[0].values["LeftFoot_gamma"][1]
out_data2[0].values["LeftFoot_gamma"][3]

(
    out_data[0].values["RightFoot_alpha"] ** 2
    + out_data[0].values["RightFoot_beta"] ** 2
    + out_data[0].values["RightFoot_gamma"] ** 2
).mean()
(
    out_data2[0].values["RightFoot_alpha"][10:] ** 2
    + out_data2[0].values["RightFoot_beta"][10:] ** 2
    + out_data2[0].values["RightFoot_gamma"][10:] ** 2
).mean()
(
    out_data2[0].values["RightFoot_alpha"][10:] ** 2
    + out_data2[0].values["RightFoot_beta"][10:] ** 2
...
yposs = list(
    filter(lambda x: x.split("_")[1] == "Yposition", out_data[0].values.columns)
)
...
####
last_index = data.values[
    (data.values["Hips_Xposition"] > 100000) | (data.values["Hips_Xposition"] < -100000)
].index[-1]
...
a = np.load(
    "inference/generated_1/transflower_expmap_finetune2/predicted_mods/aistpp_gBR_sBM_cAll_d04_mBR0_ch10.expmap_scaled_20.generated.npy"
)
...
data_pipe = jl.load(
    open("data/dance_combined/motion_expmap_cr_scaled_20_data_pipe.sav", "rb")
)
