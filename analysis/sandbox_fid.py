...
def FID(m, C, mg, Cg):
    ...
    covar_diff = (
        np.trace(C) + np.trace(Cg) - 2 * np.trace(scipy.linalg.sqrtm(np.dot(C, Cg)))
    )
...
experiments = [
    "moglow_expmap",
    "transflower_expmap",
    "transflower_expmap_finetune2_old",
    "transformer_expmap",
]
...
for i in range(5):
    ...
    for j in range(5):
        ...
        moments_file = (
            "inference/randomized_seeds/generated_"
            + str(j + 1)
            + "/transflower_expmap/predicted_mods/expmap_scaled_20.generated_"
            + stat
            + ".pkl"
        )
