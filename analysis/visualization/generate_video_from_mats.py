...
def generate_video_from_mats(
    pred_mats_file, output_folder, audio_file, trim_audio=0, fps=60, plot_mats=False
):
    ...
    smpl = SMPL(
        model_path="./aistplusplus_api", gender="MALE", batch_size=1
    )  # what if we try gender='FEMALE'?
    output = smpl.forward(
    ...
    keypoints3d = keypoints3d[
        :, :24
    ]  # the body joints (ignoring the extra head, feet and hand bones added onto it here https://github.com/vchoutas/smplx/blob/7547ee6656b942a68a97604d0cf7b6b834fad9eb/smplx/vertex_joint_selector.py)
