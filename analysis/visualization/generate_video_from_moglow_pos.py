...
def pre_process_motion_data(
    clip, init_rot=None, init_trans=None, fps=30, axis_scale=50, elev=45, azim=45
):
...
def generate_video_from_moglow_loc(
    data, control, output_folder, seq_id, audio_file, fps, trim_audio=0
):
    # import pdb;pdb.set_trace()
    clip = np.concatenate([data, control], axis=1)

    joints, translation, rot, translations = pre_process_motion_data(
        clip, fps=20, axis_scale=50, elev=45, azim=45
    )
