...
def generate_video_from_images(img_folder, video_file, framerate):
    bash_command = (
        ...
        + img_folder
        + "/img_%d.png -vcodec libx264 -crf 25  -pix_fmt yuv420p "
        + video_file
    )
...
def join_video_and_audio(video_file, audio_file, trim_audio=0):
    ...
    bash_command = (
        "ffprobe -v 0 -show_entries format=duration -of compact=p=0:nk=1 " + video_file
    )
    ...
    bash_command = (
        "ffmpeg -y -i " + video_file + " -i " + new_audio_file + " " + video_file2
    )
    run_bash_command(bash_command)
