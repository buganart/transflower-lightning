...
def get_rot_matrices_from_euler(joint_traj):
    return np.stack(
        [
            np.concatenate(
                [
                    R.from_euler("xyz", euler_angles).as_matrix().flatten()
                    for euler_angles in np.array(joint_angles).reshape(-1, 3)
                ]
            )
...
candidate_motion_files = sorted(
    data_path.glob("**/*.pkl"), key=lambda path: path.parent.__str__()
)
