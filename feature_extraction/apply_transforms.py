...
parser.add_argument(
    ...
    type=str,
    default="mel",
    help="coma separated list of names of features to combine",
)
parser.add_argument(
    "--transform_name",
    metavar="",
...
parser.add_argument(
    ...
    type=int,
    default=2,
    help="number of pca dimensions to keep, if applying pca transform",
)
...
for i in tasks:
    ...
    if replace_existing or not os.path.isfile(new_feature_file):
        ...
        transform = pickle.load(
            open(data_path.joinpath(feature_name + "_" + transform_name + ".pkl"), "rb")
        )
        pickle.dump(
            transform, open(data_path.joinpath(new_feature_name + "_scaler.pkl"), "wb")
        )
