...
def extract_features_spectral_flux(music_file, tgt_fps=20):
    ...
    spec = madmom.audio.spectrogram.Spectrogram(
        music_file, fps=tgt_fps, filterbank=filtbank, num_channels=1
    )
...
def extract_features_multi_mel(
    y, sr=44100.0, hop=512, nffts=[1024, 2048, 4096], mel_dim=100
):
    featuress = []
    for nfft in nffts:
        mel = librosa.feature.melspectrogram(
            y=y, sr=sr, n_mels=mel_dim, n_fft=nfft, hop_length=hop
        )  # C2 is 65.4 Hz
...
def extract_features_hybrid(y, sr, hop, mel_dim=12, window_mult=1):
    hop -= (
        hop % 32
    )  #  Chroma CQT only accepts hop lengths that are multiples of 32, so this ensures that condition is met
    window = (
        window_mult * hop
    )  # Fast Fourier Transform Window Size is a multiple (default 1) of the hop
    y_harm, y_perc = librosa.effects.hpss(y)
    mels = librosa.feature.melspectrogram(
        y=y_perc, sr=sr, n_fft=window, hop_length=hop, n_mels=mel_dim, fmax=65.4
    )  # C2 is 65.4 Hz
    cqts = librosa.feature.chroma_cqt(
        y=y_harm,
        sr=sr,
        hop_length=hop,
        norm=np.inf,
...
def extract_features_mel(y, sr, hop, mel_dim=100):
    mel = librosa.feature.melspectrogram(
        y=y, sr=sr, n_mels=mel_dim, hop_length=hop
    )  # C2 is 65.4 Hz
    features = librosa.power_to_db(mel, ref=np.max)
    return features
...
def extract_features_chroma(y, sr, state_times):
    ...
    state_frames = librosa.core.time_to_frames(
        state_times, sr=sr
    )  # Default hop length of 512
    # TODO: CHANGE THIS TO BECOME LIKE HYBRID IF WE ARE TO EVER USE THIS
    beat_chroma = librosa.util.sync(
        chromagram, state_frames, aggregate=np.median, pad=True, axis=-1
    )
    return beat_chroma
...
def extract_features_mfcc(y, sr, state_times):
    ...
    beat_mfcc = librosa.util.sync(
        mfcc, state_frames, aggregate=np.median, pad=True, axis=-1
    )
    return beat_mfcc
