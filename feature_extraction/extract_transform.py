...
parser.add_argument(
    "data_path", type=str, help="Directory contining Beat Saber level folders"
)
parser.add_argument(
    "--feature_name", metavar="", type=str, default="mel", help="mel, chroma, multi_mel"
)
parser.add_argument(
    "--transforms",
    metavar="",
    type=str,
    default="scaler",
    help="comma-separated lists of transforms to extract (scaler,pca_transform)",
)
...
candidate_files = sorted(
    data_path.glob("**/*" + feature_name + ".npy"),
...
for transform in transforms:
    if transform == "2moments":
        ...
        pickle.dump(
            (m, C), open(data_path.joinpath(feature_name + "_2moments.pkl"), "wb")
        )
        ...
        m = np.mean(features, 0)
        pickle.dump(
            (m, C), open(data_path.joinpath(feature_name + "_2moments_ext.pkl"), "wb")
        )
    elif transform == "scaler":
        scaler = preprocessing.StandardScaler().fit(features)
        pickle.dump(
            scaler, open(data_path.joinpath(feature_name + "_scaler.pkl"), "wb")
        )
        ...
        pickle.dump(
            pca_transform,
            open(data_path.joinpath(feature_name + "_pca_transform.pkl"), "wb"),
        )
    else:
