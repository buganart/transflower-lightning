...
opt = json.loads(
    open(
        ROOT_DIR.__str__() + "/feature_extraction/" + experiment_name + "opt.json", "r"
    ).read()
)
...
candidate_feature_files = sorted(
    data_path.glob("**/*" + audio_format + "_multi_mel_80.npy"),
...
for i in tasks:
    ...
    if replace_existing or not os.path.isfile(features_file):
        ...
        first_samples = torch.full(
            (1, opt.output_channels, receptive_field // 2),
            START_STATE,
            dtype=torch.float,
        )
        ...
        features = features[0]
        features = ResampleLinear1D(
            features, int(np.floor(features.shape[0] * 0.01 * fps))
        )
