...
def hz2bark(f):
    ...
    raise NotImplementedError(
        "please check this function, it produces " "negative values"
    )
...
class Filterbank(np.ndarray):
    ...
    def __new__(cls, data, bin_frequencies):
        ...
        if len(bin_frequencies) != obj.shape[0]:
            raise ValueError(
                "`bin_frequencies` must have the same length as "
...
class MelFilterbank(Filterbank):
    ...
    def __new__(
        ...
        **kwargs
        ...
        # convert to bins
        bins = frequencies2bins(
            frequencies, bin_frequencies, unique_bins=unique_filters
        )
...
class BarkFilterbank(Filterbank):
    ...
    def __new__(
        ...
        **kwargs
        ...
        # convert to bins
        bins = frequencies2bins(
            frequencies, bin_frequencies, unique_bins=not unique_filters
        )
...
class LogarithmicFilterbank(Filterbank):
    ...
    def __array_finalize__(self, obj):
        if obj is None:
            return
        # set default values here
        self.num_bands_per_octave = getattr(
            obj, "num_bands_per_octave", self.NUM_BANDS_PER_OCTAVE
        )
...
class RectangularFilterbank(Filterbank):
    ...
    def __new__(
        ...
        fb = np.zeros(
            (len(bin_frequencies), len(crossover_frequencies) + 1), dtype=FILTER_DTYPE
        )
...
class PitchClassProfileFilterbank(Filterbank):
    ...
    def __init__(
        self, bin_frequencies, num_classes=CLASSES, fmin=FMIN, fmax=FMAX, fref=A4
    ):
        # this method is for documentation purposes only
        pass

    def __new__(
        cls, bin_frequencies, num_classes=CLASSES, fmin=FMIN, fmax=FMAX, fref=A4
    ):
        # pylint: disable=arguments-differ
...
class SemitoneBandpassFilterbank(object):
    ...
    def __init__(
        ...
        for freq, sample_rate in zip(self.center_frequencies, self.band_sample_rates):
            ...
            self.filters.append(
                ellip(
                    order, passband_ripple, stopband_rejection, freqs, btype="bandpass"
                )
            )
