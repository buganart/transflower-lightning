...
CHORD_ANN_DTYPE = [("start", np.float), ("end", np.float), ("chord", CHORD_DTYPE)]
...
class ChordEvaluation(EvaluationMixin):
    ...
    def root(self):
        ...
        return np.average(
            score_root(self.detections, self.annotations), weights=self.durations
        )
    ...
    def majmin(self):
        ...
        return np.average(
            score_exact(det_triads, ann_triads), weights=self.durations * majmin_sel
        )
