...
class ParallelProcessor(SequentialProcessor):
    ...
    def process(self, data, **kwargs):
        ...
        return list(
            self.map(_process, zip(self.processors, it.repeat(data), it.repeat(kwargs)))
        )
...
class IOProcessor(OutputProcessor):
    ...
    def __getitem__(self, index):
        ...
        if index == 0:
            ...
            raise IndexError(
                "Only `in_processor` at index 0 and "
...
def io_arguments(parser, output_suffix=".txt", pickle=True, online=False):
    ...
    # add general options
    parser.add_argument(
        "-v", dest="verbose", action="count", help="increase verbosity level"
    )
    # add subparsers
    ...
    if online:
        ...
        sp.add_argument(
            ...
            type=int,
            default=None,
            help="PyAudio device index of the "
