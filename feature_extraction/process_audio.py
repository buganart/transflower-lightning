...
parser.add_argument(
    ...
    type=str,
    default="mel",
    help="mel, chroma, multi_mel, spectralflux, madmombeats. Comma separated",
)
parser.add_argument(
    ...
    type=str,
    default=None,
    help="name for the combined features, if several",
)
...
candidate_audio_files = sorted(
    data_path.glob("**/*." + audio_format), key=lambda path: path.parent.__str__()
)
    ...
    if replace_existing or not os.path.isfile(features_file):
        ...
        for feature_name in feature_names:
            ...
            if feature_name == "chroma":
                ...
                features = extract_features_mel(
                    y_wav, sr, hop, mel_dim=mel_feature_size
                ).transpose(1, 0)[1:]
                ...
                features = extract_features_multi_mel(
                    ...
                    mel_dim=mel_feature_size,
                )
            elif (
                feature_name == "spectralflux"
            ):  # actually this is the same as envelope I think
