...
parser.add_argument(
    "--files_extension",
    type=str,
    help="file extension (the stuff after the base filename) to match",
)
parser.add_argument(
    "--name_processing_function",
    type=str,
    default="dance_style",
    help="function for processing the names",
)
...
files = sorted(
    data_path.glob("**/*." + files_extension), key=lambda path: path.parent.__str__()
)
