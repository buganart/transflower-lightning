...
parser.add_argument(
    "--do_mirror",
    action="store_true",
    help="whether to augment the data with mirrored motion",
)
...
def extract_joint_angles(files):
    if len(files) > 0:
        ...
        if rank == 0:
            jl.dump(
                data_pipe, os.path.join(data_path, "motion_" + param + "_data_pipe.sav")
            )
...
candidate_motion_files = sorted(
    data_path.glob("**/*.bvh"), key=lambda path: path.parent.__str__()
)
