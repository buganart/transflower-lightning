...
def convert_smpl(filepath, dest_dir, result_filename, framerate):
    ...
    for smpl_joint, unity_joint in smpl_joints_to_unity_joints.items():
        out_data[0, :, unity_joints_vals.get_loc(unity_joint + "_alpha")] = smpl_angs[
            :, smpl_joints.index(smpl_joint) * 3
        ]
        out_data[0, :, unity_joints_vals.get_loc(unity_joint + "_beta")] = smpl_angs[
            :, smpl_joints.index(smpl_joint) * 3 + 1
        ]
        out_data[0, :, unity_joints_vals.get_loc(unity_joint + "_gamma")] = smpl_angs[
            :, smpl_joints.index(smpl_joint) * 3 + 2
        ]
    ...
    out_data[0, :, unity_joints_vals.get_loc("Hips_Xposition")] = (
        smpl_trans[:, 0] / relative_scale
    )
    out_data[0, :, unity_joints_vals.get_loc("Hips_Yposition")] = (
        smpl_trans[:, 1] / relative_scale + vertical_offset
    )
    out_data[0, :, unity_joints_vals.get_loc("Hips_Zposition")] = (
        smpl_trans[:, 2] / relative_scale
    )
