...
from analysis.visualization.generate_video_from_expmaps import (
    generate_video_from_expmaps,
)
from analysis.visualization.generate_video_from_moglow_pos import (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--seeds", type=str, help="in the format: mod,seq_id;mod,seq_id"
    )
    parser.add_argument(
        "--seeds_file", type=str, help="file from which to choose a random seed"
    )
    ...
    if args.seeds is not None:
        seeds = {
            mod: seq
            for mod, seq in [tuple(x.split(",")) for x in args.seeds.split(";")]
        }
    else:
        seeds = {}
    ...
    if seq_id is None:
        temp_base_filenames = [
            x[:-1] for x in open(data_dir + "/base_filenames_test.txt", "r").readlines()
        ]
    ...
    if len(predicted_mods) == 0:
        ...
        for i, mod in enumerate(output_mods):
            ...
            if args.generate_video:
                trim_audio = (
                    output_time_offsets[i] / fps
                )  # converting trim_audio from being in frames (which is more convenient as thats how we specify the output_shift in the models), to seconds
                ...
                if mod == "joint_angles_scaled":
                    ...
                    mod == "expmap_scaled"
                    or mod == "expmap_scaled_20"
                    or mod == "expmap_cr_scaled_20"
                ):
                    ...
                    if args.use_scalers:
                        transform = pickle.load(
                            open(data_dir + "/moglow_control_scaled_scaler.pkl", "rb")
                        )
