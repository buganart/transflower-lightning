...
class BaseModel(LightningModule):
    ...
    def parse_base_arguments(self):
        ...
        self.output_time_offsets = [
            int(x) for x in str(self.opt.output_time_offsets).split(",")
        ]
        self.input_time_offsets = [
            int(x) for x in str(self.opt.input_time_offsets).split(",")
        ]
        ...
        if self.opt.input_num_tokens is None:
            ...
            self.input_num_tokens = [
                int(x) for x in self.opt.input_num_tokens.split(",")
            ]
        ...
        if self.opt.output_num_tokens is None:
            ...
            self.output_num_tokens = [
                int(x) for x in self.opt.output_num_tokens.split(",")
            ]
    ...
    def generate(self, features, teacher_forcing=False, ground_truth=False):
        output_seq = autoregressive_generation_multimodal(
            features,
            self,
            autoreg_mods=self.output_mods,
