...
class ConditionalDiscreteVAEVision(nn.Module):
    ...
    def forward(
        ...
        device, num_tokens, image_shape, kl_div_loss_weight = (
            img.device,
            self.num_tokens,
            self.image_shape,
        ...
        assert (
            img.shape[-1] == image_shape[1] and img.shape[-2] == image_shape[0]
        ), f"input must have the correct image size {image_shape[0]}x{image_shape[1]}"
...
class ConditionalDiscreteVAE(nn.Module):
    def __init__(
        ...
        for (enc_in, enc_out), (dec_in, dec_out) in zip(enc_chans_io, dec_chans_io):
            enc_layers.append(
                nn.Sequential(
                    nn.Conv2d(
                        enc_in, enc_out, kernel_shape, stride=1, padding=padding_shape
                    ),
                    nn.ReLU(),
                )
            )
            dec_layers.append(
                nn.Sequential(
                    nn.ConvTranspose2d(
                        dec_in, dec_out, kernel_shape, stride=1, padding=padding_shape
                    ),
                    nn.ReLU(),
                )
            )
        ...
        if num_resnet_blocks > 0:
            if cond_vae:
                dec_layers.insert(
                    0, nn.Conv2d(codebook_dim + cond_dim, dec_chans[1], 1)
                )
            else:
                dec_layers.insert(0, nn.Conv2d(codebook_dim, dec_chans[1], 1))
        ...
        self.cond_upsampler = torch.nn.Upsample(
            size=input_shape
        )  # upsampler to feed the conditioning to the input of the encoder
        self.encoder = nn.Sequential(*enc_layers)
        self.decoder = nn.Sequential(*dec_layers)
        ...
        if cond_dim > 0:
            self.prior_transformer = ContDiscTransformer(
                cond_dim,
                num_tokens,
                codebook_dim,
                prior_nhead,
    ...
    def decode(self, img_seq, cond=None):
        image_embeds = self.codebook(img_seq)
        b, n, d = image_embeds.shape
        h = w = int(sqrt(n))
    ...
    def prior_logp(self, inputs, cond=None, return_accuracy=False, detach_cond=False):
        ...
        if len(cond.shape) == 3:
            cond = cond.reshape(
                cond.shape[0], cond.shape[1], *self.codebook_layer_shape
            )
        with torch.no_grad():
            if self.cond_vae:
        ...
        if detach_cond:
            cond = cond.detach()
        logits = self.prior_transformer(
            cond.squeeze(-1).permute(2, 0, 1), labels.permute(1, 0)
        ).permute(1, 2, 0)
    ...
    def generate(self, cond, temp=1.0, filter_thres=0.5):
        ...
        for i in range(self.latent_size):
            # print(i)
            logits = self.prior_transformer(
                cond.squeeze(-1).permute(2, 0, 1), torch.cat(tokens + [dummy], 0)
            ).permute(1, 2, 0)[:, -1, :]
    ...
    def forward(
        ...
        device, num_tokens, input_shape, kl_div_loss_weight = (
            inp.device,
            self.num_tokens,
            self.input_shape,
            self.kl_div_loss_weight,
        ...
        assert (
            inp.shape[-1] == input_shape[1] and inp.shape[-2] == input_shape[0]
        ), f"input must have the correct image size {input_shape[0]}x{input_shape[1]}. Instead got {inp.shape[0]}x{inp.shape[1]}"
...
class ContDiscTransformer(nn.Module):
    def __init__(
        ...
        self.transformer = EncDecTransformerModel(
            tgt_num_tokens,
            src_d,
            tgt_emb_dim,
            nhead,
