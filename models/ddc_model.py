...
class DDCModel(nn.Module):
                ...
                [
                    {
                        ...
                        "lr": 2 * opt.learning_rate,
                    },  # bias parameters change quicker - no weight decay is applied
                    {
                        "params": [
                            param
                            for name, param in self.net.named_parameters()
                    ...
                    },  # filter parameters have weight decay
                ]
            )
        ]
        self.loss_ce = None
    ...
    def forward(self):
        ...
        self.metric_accuracy = (torch.argmax(x, 1) == self.target).sum().float() / len(
            self.target
        )
    ...
    def prepare_input(self, y):
            ...
            (
                np.zeros(
                    (
                        y.shape[0],
                        y.shape[1],
                        receptive_field + self.opt.time_shifts // 2,
                    )
                ),
                y,
            ),
        ...
        y = np.concatenate(
            (y, np.zeros((y.shape[0], y.shape[1], self.opt.time_shifts // 2))), 2
        )
        input_windowss = []
        ...
        for ii in range(-time_shifts // 2, time_shifts // 2 + 1):
            input_windows = [
                y[
                    :,
                    :,
                    self.opt.time_shifts // 2
                    + ii : self.opt.time_shifts // 2
                    + ii
                    + input_length,
                ]
            ]
            input_windows = torch.tensor(input_windows)
            input_windows = (input_windows - input_windows.mean()) / torch.abs(
                input_windows
            ).max()
...
class DDCNet(nn.Module):
    def __init__(self, opt):
        ...
        self.hidden_to_state = nn.Linear(opt.hidden_dim, opt.num_classes)
