...
def f(
    in_channels, out_channels, hidden_channels, cond_channels, network_model, num_layers
):
    if network_model == "transformer":
        ...
        return BasicTransformerModelCausal(
            out_channels,
    ...
    if network_model == "LSTM":
        return modules.LSTM(
            in_channels + cond_channels, hidden_channels, out_channels, num_layers
        )
...
class Glow(nn.Module):
    ...
    def forward(
        self, x=None, cond=None, z=None, eps_std=None, reverse=False, output_length=1
    ):
        if not reverse:
