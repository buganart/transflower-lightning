...
class MowgliModel(BaseModel):
    ...
    def modify_commandline_options(parser, opt):
        ...
        parser.add_argument(
            "--prior_loss_weight_warmup_epochs", type=float, default=100
        )
        parser.add_argument(
            "--max_prior_loss_weight",
            type=float,
            default=0,
            help="max value of prior loss weight during stage 1 (e.g. 0.01 is a good value)",
        )
        ...
        parser.add_argument(
            "--vae_hard",
            action="store_true",
            help="whether to use the hard one-hot vector as output and use the straight through gradient estimator, for discrete latents",
        )
        ...
        parser.add_argument(
            "--residual",
            action="store_true",
            help="whether to use the vae to predict the residual around a determnisitic mean",
        )
        parser.add_argument(
            "--use_pos_emb_output",
            action="store_true",
        ...
        parser.add_argument(
            "--prior_no_use_pos_emb",
            action="store_true",
            help="dont use positional embeddings for the prior transformer",
        )
        parser.add_argument(
            "--stage2",
            action="store_true",
            help="stage2: train the prior, rather than the VAE",
        )
        parser.add_argument(
            "--cond_vae", action="store_true", help="whether to use a conditional vae"
        )
        return parser
    ...
    def forward(self, data, temp=1.0):
        ...
        if self.opt.residual:
            for i, mod in enumerate(self.output_mods):
                ...
                residual = self.output_mod_vaes[i].generate(
                    trans_output.permute(1, 2, 0), temp=temp
                )
            ...
            for i, mod in enumerate(self.output_mods):
                ...
                output = self.output_mod_vaes[i].generate(
                    trans_output.permute(1, 2, 0), temp=temp
                )
                # import pdb;pdb.set_trace()
    ...
    def on_train_epoch_start(self):
        self.prior_loss_weight = self.opt.max_prior_loss_weight * min(
            self.current_epoch / self.opt.prior_loss_weight_warmup_epochs, 1
        )
        self.vae_temp = max(
            self.vae_temp * math.exp(-self.opt.anneal_rate * self.global_step),
            self.opt.temp_min,
        )
            ...
            if self.opt.residual:
                ...
                for i, mod in enumerate(self.output_mods):
                    ...
                    if not self.opt.stage2:
                        nll_loss += vae(
                            (self.targets[i] - predicted_mean).permute(1, 2, 0),
                            cond=latents1.permute(1, 2, 0),
                            return_loss=True,
                            temp=self.vae_temp,
                        )  # time, batch, features -> batch, features, time
                        if self.opt.max_prior_loss_weight > 0:
                            prior_loss, accuracy = vae.prior_logp(
                                (self.targets[i] - predicted_mean).permute(1, 2, 0),
                                cond=latents2.permute(1, 2, 0),
                ...
                # import pdb;pdb.set_trace()
                for i, mod in enumerate(self.output_mods):
                    output1 = self.output_mod_nets[i].forward(latent)[
                        : self.conditioning_seq_lens[i]
                    ]
                    output2 = output1
                    vae = self.output_mod_vaes[i]
                    if not self.opt.stage2:
                        loss += vae(
                            self.targets[i].permute(1, 2, 0),
                            cond=output1.permute(1, 2, 0),
                            return_loss=True,
                            temp=self.vae_temp,
                        )  # time, batch, features -> batch, features, time
                        if self.opt.max_prior_loss_weight > 0:
