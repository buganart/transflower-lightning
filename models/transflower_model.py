...
class TransflowerModel(BaseModel):
    def __init__(self, opt):
        ...
        if self.opt.conditioning_seq_lens is not None:
            self.conditioning_seq_lens = [
                int(x) for x in str(self.opt.conditioning_seq_lens).split(",")
            ]
        else:
            self.conditioning_seq_lens = [
        ...
        for i, mod in enumerate(output_mods):
            ...
            glow = FlowPlusPlus(
                scales=ast.literal_eval(opt.scales),
    ...
    def modify_commandline_options(parser, opt):
        ...
        parser.add_argument(
            "--conditioning_seq_lens",
            type=str,
            default=None,
            help="the number of outputs of the conditioning transformers to feed (meaning the number of elements along the sequence dimension)",
        )
        ...
        parser.add_argument(
            "--glow_use_attn",
            action="store_true",
            help="whether to use the internal attention for the FlowPlusPLus model",
        )
        parser.add_argument(
            "--use_transformer_nn",
            action="store_true",
        ...
        parser.add_argument(
            "--use_rel_pos_emb_inputs",
            action="store_true",
            help="whether to use T5 relative positional embeddings for input modality transformers",
        )
        parser.add_argument(
            "--use_rel_pos_emb_output",
            action="store_true",
            help="whether to use T5 relative positional embeddings for output modality transformers",
        )
        parser.add_argument(
            "--use_pos_emb_inputs",
            action="store_true",
            help="whether to use positional embeddings for output modality transformers",
        )
        parser.add_argument(
            "--use_pos_emb_output",
            action="store_true",
        ...
        parser.add_argument(
            ...
            action="store_true",
            help="whether to use positional embeddings for the coupling layer transformers",
        )
        parser.add_argument(
            "--use_rel_pos_emb_coupling",
            action="store_true",
            help="whether to use T5 relative positional embeddings for the coupling layer transformers",
        )
        parser.add_argument(
            "--cond_concat_dims",
            action="store_true",
            help="if set we concatenate along the channel dimension with with the x for the coupling layer; otherwise we concatenate along the sequence dimesion",
        )
        parser.add_argument(
            "--residual",
            action="store_true",
            help="whether to use the flow to predict the residual around a determnisitic mean",
        )
        parser.add_argument(
            "--use_rotary_pos_emb",
            action="store_true",
        ...
        if self.opt.residual:
            for i, mod in enumerate(self.output_mods):
                trans_output = self.output_mod_nets[i].forward(latent)[
                    : self.conditioning_seq_lens[i]
                ]
                trans_predicted_mean_latents = self.output_mod_nets[i].forward(latent)[
                    self.conditioning_seq_lens[i] : self.conditioning_seq_lens[i]
                    + self.output_lengths[i]
                ]
                predicted_mean = self.output_mod_mean_nets[i](
                    trans_predicted_mean_latents
                )
                residual, _ = self.output_mod_glows[i](
                    x=None, cond=trans_output.permute(1, 0, 2), reverse=True
                )
            ...
            for i, mod in enumerate(self.output_mods):
                ...
                output, _ = self.output_mod_glows[i](
                    x=None, cond=trans_output.permute(1, 0, 2), reverse=True
                )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.opt.residual:
            ...
            for i, mod in enumerate(self.output_mods):
                ...
                trans_predicted_mean_latents = trans_output[
                    self.conditioning_seq_lens[i] : self.conditioning_seq_lens[i]
                    + self.output_lengths[i]
                ]
                predicted_mean = self.output_mod_mean_nets[i](
                ...
                z, sldj = glow(
                    x=self.targets[i].permute(1, 0, 2)
                    - predicted_mean.permute(1, 0, 2),
                    cond=latents.permute(1, 0, 2),
            ...
            loss = 0
            for i, mod in enumerate(self.output_mods):
                output = self.output_mod_nets[i].forward(latent)[
                    : self.conditioning_seq_lens[i]
                ]
                glow = self.output_mod_glows[i]
                # import pdb;pdb.set_trace()
                # print(output)
                z, sldj = glow(
                    x=self.targets[i].permute(1, 0, 2), cond=output.permute(1, 0, 2)
                )  # time, batch, features -> batch, time, features
                # print(z)
                # print(sldj)
