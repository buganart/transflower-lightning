...
class BasicTransformerModelCausal(nn.Module):
    def __init__(
        ...
        self.model = BasicTransformerModel(
            self,
            dout,
            dinp,
            nhead,
...
class EncDecTransformerModel(nn.Module):
    def __init__(
        ...
        if not use_x_transformers:
            ...
            self.transformer = EncDecXTransformer(
                enc_dim_in=src_d,
                enc_dim_out=tgt_d,
                dec_din_in=tgt_d,
                edec_dim_out=dout,
        ...
        if not self.use_x_transformers:
            ...
            if self.use_pos_emb:
                ...
                output = self.transformer(
                    src=src,
                    tgt=tgt,
                    src_mask=self.src_pos_emb,
                    tgt_mask=tgt_pos_emb + tgt_mask,
                )
...
class EncDecXTransformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.encoder = ContinuousTransformerWrapper(
            **enc_transformer_kwargs, attn_layers=Encoder(**enc_kwargs)
        )
    ...
    def generate(self, seq_in, seq_out_start, seq_len, src_mask=None):
        ...
        return self.decoder.generate(
            seq_out_start, seq_len, context=encodings, context_mask=src_mask
        )
