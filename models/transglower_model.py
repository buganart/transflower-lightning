...
class TransglowerModel(BaseModel):
    def __init__(self, opt):
        ...
        for i, mod in enumerate(output_mods):
            net = BasicTransformerModel(
                ...
                input_length=sum(input_seq_lens),
            ).to(self.device)
            name = "_output_" + mod
            setattr(self, "net" + name, net)
    ...
    def parse_base_arguments(self):
        ...
        self.output_seq_lens = [
            int(x) for x in str(self.opt.output_seq_lens).split(",")
        ]
        ...
        if len(self.output_time_offsets) < len(self.output_mods):
            if len(self.output_time_offsets) == 1:
                self.output_time_offsets = self.output_time_offsets * len(
                    self.output_mods
                )
            else:
                raise Exception(
                    "number of output_time_offsets doesnt match number of output_mods"
                )
        ...
        if len(self.input_time_offsets) < len(self.input_mods):
            if len(input_time_offsets) == 1:
                ...
                raise Exception(
                    "number of input_time_offsets doesnt match number of input_mods"
                )
    ...
    def forward(self, data):
        ...
        if self.opt.residual:
            for i, mod in enumerate(self.output_mods):
                ...
                trans_predicted_mean_latents = trans_predicted_mean_latents.reshape(
                    trans_predicted_mean_latents.shape[0],
                    trans_predicted_mean_latents.shape[1]
                    * trans_predicted_mean_latents.shape[2],
                    trans_predicted_mean_latents.shape[3],
                )
            ...
            for i, mod in enumerate(self.output_mods):
                ...
                trans_output = trans_output.reshape(
                    trans_output.shape[0],
                    trans_output.shape[1] * trans_output.shape[2],
                    trans_output.shape[3],
                )
                glow = self.output_mod_glows[i]
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.opt.residual:
            ...
            for i, mod in enumerate(self.output_mods):
                ...
                # import pdb;pdb.set_trace()
                z, nll = glow(
                    x=self.targets[i] - predicted_mean, cond=latents
