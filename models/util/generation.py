...
def autoregressive_generation_multimodal(
    features, model, autoreg_mods=[], teacher_forcing=False, ground_truth=False
):
    inputs_ = []
    ...
    for i, mod in enumerate(input_mods):
        input_tmp.append(
            inputs_[i].clone()[
                input_time_offsets[i] : input_time_offsets[i] + input_lengths[i]
            ]
        )
        ...
        for t in range(sequence_length - max(input_lengths) + 1):
            ...
            if t == 0:
                ...
                for i, mod in enumerate(output_mods):
                    ...
                    output_seq[i] = torch.cat(
                        [output_seq[i], output[:1].detach().clone()]
                    )
            ...
            if t < sequence_length - 1:
                for i, mod in enumerate(input_mods):
                    if mod in autoreg_mods:
                        ...
                        if teacher_forcing:
                            input_tmp[i] = torch.cat(
                                [
                                    input_tmp[i][1:],
                                    inputs_[i][
                                        t
                                        + input_time_offsets[i]
                                        + input_lengths[i] : t
                                        + input_time_offsets[i]
                                        + input_lengths[i]
                                        + 1
                            ...
                            # import pdb;pdb.set_trace()
                            input_tmp[i] = torch.cat(
                                [input_tmp[i][1:], output[:1].detach().clone()], 0
                            )
                            ...
                            print(
                                torch.mean(
                                    (
                                        inputs_[i][
                                            t
                                            + output_time_offsets[j] : t
                                            + output_time_offsets[j]
                                            + 1
                                        ]
                                        - outputs[j][:1].detach().clone()
                                    )
                                    ** 2
                                )
                            )
