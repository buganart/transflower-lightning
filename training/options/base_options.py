...
class BaseOptions:
    def __init__(self):
        ...
        parser.add_argument(
            "--do_validation",
            action="store_true",
            help="whether to do validation steps during training",
        )
        parser.add_argument(
            "--do_testing",
            action="store_true",
            help="whether to do evaluation on test set at the end of training",
        )
        parser.add_argument(
            "--skip_training",
            action="store_true",
            help="whether to not do training (only useful when doing just testing)",
        )
        parser.add_argument(
            "--do_tuning",
            action="store_true",
            help="whether to not do the tuning phase (e.g. to tune learning rate)",
        )
        ...
        parser.add_argument(
            ...
            type=str,
            default="transformer",
            help="The network model used for beatsaberification",
        )
        ...
        parser.add_argument(
            "--fork_processes",
            action="store_true",
            help="Set method to create dataloader child processes to fork instead of spawn (could take up more memory)",
        )
        parser.add_argument(
            "--find_unused_parameters",
            action="store_true",
            help="option used with DDP which allows having parameters which are not used for producing the loss. Setting it to false is more efficient, if this option is not needeed",
        )
        ### CHECKPOINTING STUFF
        parser.add_argument(
            "--checkpoints_dir",
        ...
        parser.add_argument(
            "--load_weights_only",
            action="store_true",
            help="if specified, we load the model weights from the last checkpoint for the specified experiment, WITHOUT loading the optimizer parameters! (allows to continue traning while changing the optimizer)",
        )
        parser.add_argument(
            "--no_load_hparams",
            action="store_true",
            help="if specified, we dont load the saved experiment hparams when doing continue_train",
        )
        parser.add_argument(
            "--ignore_in_state_dict",
            type=str,
            default="",
            help="substring to match in state dict, to then ignore the corresponding saved weights. Sometimes useful for models where only some part was trained e.g.",
        )
        parser.add_argument(
            "--only_load_in_state_dict",
            type=str,
            default="",
            help="substring to match in state dict, to then only load the corresponding saved weights. Sometimes useful for models where only some part was trained e.g.",
        )
    ...
    def gather_options(self, parse_args=None):
        ...
        if opt.continue_train and not opt.no_load_hparams:
            ...
            try:
                ...
                print(
                    "checkpoint file not found. Probably trying continue_train on an experiment with no checkpoints"
                )
                raise
        ...
        if opt.hparams_file is not None:
            ...
            hparams_json2 = {
                k: v for k, v in hparams_json.items() if (v != False and k in defaults)
            }
