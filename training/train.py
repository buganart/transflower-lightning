...
if __name__ == "__main__":
    ...
    if "tpu_cores" in vars(opt) and opt.tpu_cores is not None and opt.tpu_cores > 0:
        ...
        print("DDPPlugin")
        plugins = DDPPlugin(
            find_unused_parameters=opt.find_unused_parameters, num_nodes=opt.num_nodes
        )
    ...
    logger = TensorBoardLogger(
        opt.checkpoints_dir, name=opt.experiment_name, default_hp_metric=False
    )
        ...
        if opt.load_weights_only:
            ...
            if opt.only_load_in_state_dict != "":
                state_dict = {
                    ...
                    if (opt.only_load_in_state_dict in k)
                }
                load_strict = False
            if opt.ignore_in_state_dict != "":
                state_dict = {
                    k: v
                    for k, v in state_dict.items()
                    if not (opt.ignore_in_state_dict in k)
                }
                load_strict = False
